...
/**
 * Current Emmet config
 * NB: use singleton here since options should be configured globally,
 * as in editors like Sublime Text or Nova.
 * In future, config could be per editor, if required
 */
const currentConfig: Partial<EmmetConfig> = {};

/**
 * Returns current Emmet config
 */
export function getEmmetConfig(): EmmetConfig {
    return { ...currentConfig, ...defaultConfig };
}

/**
 * Updates global Emmet config
 */
export function setEmmetConfig(config: Partial<EmmetConfig>): void {
